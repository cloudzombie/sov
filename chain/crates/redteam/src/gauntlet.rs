...
impl GauntletReport {
    ...
    pub fn xus(g: Option<u128>) -> String {
        g.map(|v| format!("{:.8}", v as f64 / 1e8)).unwrap_or_else(|| "?".into())
    }
}
...
fn expect_rejected(client: &RpcClient, name: &'static str, stx: &SignedTransaction) -> Outcome {
    match client.submit_transaction(stx) {
        Err(RpcClientError::Rpc { message, .. }) => Outcome::defended(CAT, name, format!("REFUSED — {}", trim(&message))),
        ...
        Ok(id) => Outcome::vulnerable(CAT, name, format!("ADMITTED — a pot-draining tx entered the mempool ({})", short(&id.to_hex()))),
    }
}
...
fn trim(s: &str) -> String {
    ...
    if first.len() > 120 { format!("{}…", &first[..119]) } else { first.to_string() }
}
...
pub fn probe_gauntlet(rpc_target: &str) -> GauntletReport {
    ...
    let mut report = GauntletReport {
        pot: POT.to_string(),
        is_mainnet: chain_id.as_deref().map(|c| c.contains("mainnet")).unwrap_or(false),
        chain_id,
        balance_before: None,
        balance_after: None,
        outcomes: Vec::new(),
    ...
    let Ok(before) = client.balance(&pot) else {
        report.error = Some(format!("node unreachable at {rpc_target}, or pot not found"));
        return report;
    };
    ...
    report.outcomes.push(expect_rejected(&client, "forge a spend with the wrong key", &drain(&Keypair::hybrid_from_seed([201; 32]), 0, sink.clone(), whole)));

    // 2. A structurally-valid drain, then corrupt the signature (both halves).
    let mut forged = drain(&Keypair::hybrid_from_seed([202; 32]), 0, sink.clone(), whole);
    forged.signature = tamper_signature(tamper_signature(forged.signature, Half::Ed25519), Half::MlDsa);
    report.outcomes.push(expect_rejected(&client, "forge the signature", &forged));

    // 3. Corrupt ONLY the post-quantum half (keep Ed25519 valid) — the PQ conjunction.
    let mut pq = drain(&Keypair::hybrid_from_seed([203; 32]), 0, sink.clone(), whole);
    pq.signature = tamper_signature(pq.signature, Half::MlDsa);
    report.outcomes.push(expect_rejected(&client, "forge only the post-quantum half", &pq));

    // 4. Sign a 1-XUS drain, then bump it to the whole pot AFTER signing (malleability).
    let mut mal = drain(&Keypair::hybrid_from_seed([204; 32]), 0, sink.clone(), Balance::from_sov(1).unwrap());
    mal.transaction.action = Action::Transfer { to: sink.clone(), amount: whole };
    report.outcomes.push(expect_rejected(&client, "edit the amount after signing", &mal));

    // 5. A zeroed / empty signature.
    let mut zero = drain(&Keypair::hybrid_from_seed([205; 32]), 0, sink.clone(), whole);
    zero.signature = Signature::V1Ed25519([0; 64]);
    report.outcomes.push(expect_rejected(&client, "empty (zeroed) signature", &zero));

    // 6. Brute-force futility: many distinct random keys, none is the pot's.
    let mut brute_rejected = 0u32;
    for seed in 20u8..28 {
        let tx = drain(&Keypair::hybrid_from_seed([seed; 32]), 0, sink.clone(), whole);
        if client.submit_transaction(&tx).is_err() {
            brute_rejected += 1;
        }
    }
    report.outcomes.push(if brute_rejected == 8 {
        Outcome::defended(CAT, "brute-force 8 random keys", "all 8 refused — only the key whose hash IS the account can spend it")
    } else {
        Outcome::vulnerable(CAT, "brute-force 8 random keys", format!("{}/8 forged spends were admitted", 8 - brute_rejected))
    });
    ...
    .unwrap();
    report.outcomes.push(expect_rejected(&client, "seize via RotateKey (wrong key)", &seize));

    // 8. Overspend/overflow drain (~u128::MAX) — probe the arithmetic path too.
    report.outcomes.push(expect_rejected(&client, "overflow drain (~u128::MAX)", &drain(&Keypair::hybrid_from_seed([207; 32]), 0, sink.clone(), Balance::from_grains(u128::MAX))));

    // 9. Malformed pot-drain: a hand-mangled payload naming the pot as signer.
    let mut raw = to_value(drain(&Keypair::hybrid_from_seed([208; 32]), 0, sink.clone(), whole)).unwrap();
    raw["signature"] = json!("not-a-signature");
    report.outcomes.push(match client.call("sov_submitTransaction", raw) {
        Err(RpcClientError::Rpc { message, .. }) => Outcome::defended(CAT, "malformed pot-drain payload", format!("REFUSED at decode — {}", trim(&message))),
        Err(RpcClientError::Io(e)) => Outcome::info(CAT, "malformed pot-drain payload", format!("could not reach node: {e}")),
        Err(e) => Outcome::defended(CAT, "malformed pot-drain payload", format!("REFUSED — {}", trim(&e.to_string()))),
        Ok(_) => Outcome::vulnerable(CAT, "malformed pot-drain payload", "the node accepted a malformed pot-draining payload"),
    });

    // 10. Replay a forged drain twice — no second bite either.
    let replay = drain(&Keypair::hybrid_from_seed([209; 32]), 0, sink.clone(), whole);
    let _ = client.submit_transaction(&replay);
    report.outcomes.push(expect_rejected(&client, "replay the forged drain", &replay));
    ...
    let thief_credited = client.balance(&sink).map(|b| b.grains() > 0).unwrap_or(false);
    report.outcomes.push(if report.pot_intact() && !thief_credited {
        Outcome::defended(CAT, "pot conservation", format!("intact — {} XUS still in the pot, no thief credited", GauntletReport::xus(report.balance_after)))
    } else {
        Outcome::vulnerable(CAT, "pot conservation", "THE POT MOVED — value left the account or a thief was credited")
    });
