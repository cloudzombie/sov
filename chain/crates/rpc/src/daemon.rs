...
/// Bit 3 must open clear of bit 2's ENTIRE window, so the two activations can
/// never share a signaling period (which would make one release's fleet lead
/// time depend on the other's). Checked at COMPILE time, because these are
/// consensus-coordinating heights: a build that violates it must not exist.
const _: () = assert!(
    MAINNET_TX_TIMESTAMP_START_HEIGHT >= MAINNET_SHIELDED_V2_START_HEIGHT + 3 * 288
        && MAINNET_TX_TIMESTAMP_START_HEIGHT % 288 == 0
);
...
mod tests {
    ...
    fn mempool_histogram_info_and_queued_flag_over_the_real_socket() {
        ...
        // Per-transaction CONTENTS with arrival timing — the "first seen by
        // this node" feed. Read-only, node-local, non-consensus.
        let txs_res = client
            .call("sov_getMempoolTxs", serde_json::json!({}))
            .unwrap();
        assert_eq!(txs_res.get("txCount"), Some(&serde_json::json!(1)));
        assert_eq!(txs_res.get("queuedCount"), Some(&serde_json::json!(1)));
        assert_eq!(
            txs_res.get("limit"),
            Some(&serde_json::json!(crate::MAX_MEMPOOL_TX_PAGE)),
            "an unspecified limit defaults to the hard cap"
        );
        assert_eq!(txs_res.get("hasMore"), Some(&serde_json::json!(false)));
        let entries = txs_res.get("txs").unwrap().as_array().unwrap();
        assert_eq!(entries.len(), 2, "one ready + one queued");

        // READY first, and each entry carries the full contracted shape.
        let ready_entry = &entries[0];
        assert_eq!(ready_entry.get("state"), Some(&serde_json::json!("ready")));
        assert_eq!(
            ready_entry.get("signer"),
            Some(&serde_json::json!("val01.node.sov"))
        );
        assert_eq!(ready_entry.get("nonce"), Some(&serde_json::json!(0)));
        assert_eq!(
            ready_entry.get("tipGrains"),
            Some(&serde_json::json!("0")),
            "decimal-string grains, the codebase convention"
        );
        assert_eq!(
            ready_entry
                ...
                .map(str::len),
            Some(64),
            "the tx id is bare hex — the same string sov_getReceipt accepts"
        );
        assert!(
            ready_entry
                .get("sizeBytes")
        ...
        assert!(ready_entry.get("weight").unwrap().is_u64());
        // The ARRIVAL TIME: milliseconds since THIS node admitted it. An
        // observation, never a self-reported creation time — and it comes off
        // the same clock `oldestPendingAgeMs` above reads, so the two views can
        // never disagree.
        let age = ready_entry
            .get("ageMs")
            .and_then(serde_json::Value::as_u64)
            .expect("every entry discloses how long this node has held it");
        let oldest = info
        ...
        // not on fees.
        let queued_entry = &entries[1];
        assert_eq!(
            queued_entry.get("state"),
            Some(&serde_json::json!("queued"))
        );
        assert_eq!(queued_entry.get("nonce"), Some(&serde_json::json!(2)));

        // The limit is a HARD cap, honored below and above the ceiling, and
        // paging advertises more.
        let one = client
            .call("sov_getMempoolTxs", serde_json::json!({"limit": 1}))
            .unwrap();
        assert_eq!(one.get("txs").unwrap().as_array().unwrap().len(), 1);
        assert_eq!(one.get("hasMore"), Some(&serde_json::json!(true)));
        let over = client
            .call(
                "sov_getMempoolTxs",
                serde_json::json!({"limit": 10_000_000u64}),
            )
            .unwrap();
        assert_eq!(
            over.get("limit"),
            Some(&serde_json::json!(crate::MAX_MEMPOOL_TX_PAGE)),
            "an absurd limit is clamped to the cap, never honored"
        );
        let paged = client
            .call("sov_getMempoolTxs", serde_json::json!({"offset": 1}))
            .unwrap();
        assert_eq!(paged.get("txs").unwrap().as_array().unwrap().len(), 1);
        assert_eq!(
            paged.get("txs").unwrap()[0].get("state"),
            Some(&serde_json::json!("queued")),
            "offset skips into the queued region"
        );
