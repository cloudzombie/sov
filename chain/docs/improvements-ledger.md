...
| **Perpetual founder/dev tax, no burn** | ✅ | Every coinbase AND every fee splits **5% founder / 2% dev / 93% miner** (`tax_primary_bps`/`tax_secondary_bps` + recipients on `MiningPolicy`, genesis-validated ≤100%); the deflationary fee-burn is REMOVED entirely. Supply invariant is now `supply == genesis + mined` (no `− burned`) — hard-capped but not deflationary. Recipients are placeholders until the real mainnet addresses are hardcoded |
